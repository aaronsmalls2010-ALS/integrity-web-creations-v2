...
export function invoiceEmail(inv: any, baseUrl: string) {
  ...
  const htmlBody = shell(`
    ...
    <p>Your invoice <strong>${esc(inv.invoice_number)}</strong> for <strong>${formatUSD(inv.total_cents)}</strong> is ready${inv.due_date ? `, due <strong>${inv.due_date}</strong>` : ''}.</p>
    ...
    ${issuer.payment_instructions ? `<p style="font-size:13px;color:#475569">${esc(issuer.payment_instructions)}</p>` : ''}
